...
Модуль поддерживает две команды:
* `.dl <ссылка>` (или ответ `.dl` на сообщение) — скачивает медиа с использованием стандартного текстового обновления сообщения на месте (без отправки GIF, чтобы не засорять вкладку «Сохраненные гифки»).
* `.dli <ссылка>` (или ответ `.dli` на сообщение) — скачивает медиа через интерактивную инлайн-форму Hikka с отображением GIF-анимаций статуса.
